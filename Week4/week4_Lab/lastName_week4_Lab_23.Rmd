...
df <- read.csv("data/Police_calls_for_service.csv")
str(df)
...
register_google(key <-"AIzaSyAn4T_cRWEmBYnLchJ14hBMBvnt4XxwMx4")
...
#Having to run this on 5400 addresses is ridiculous and takes far too much time to compile.
#The example in the picture has only 200 observations, why isn't that the case for the data provided?
#I have to shorten the data.
df <- df[1:200,]
df$Coordinates <- geocode(df$Address)
...
```{r, "Step 1.5"}
# Write your code below.
Tampa_crimemap <- simpleMap + geom_point(data=df,aes(x=Coordinates$lon, y=Coordinates$lat, color=Description))
Tampa_crimemap
